...
<%if isNumeriek(page.iParentID) then%><tr><td class=QSlabel><%=l("isattachedto")%>:</td><td><b><%=page.parentPage.sTitle%></b></td></tr><%end if%><tr><td valign=middle class=QSlabel><%=l("title")%>:*</td><td valign=middle><%if secondAdmin.bPageTitle then%><input type=text maxlength=100 size=50 name=sTitle value="<%= quotRep(page.sTitle)%>" /><%else
