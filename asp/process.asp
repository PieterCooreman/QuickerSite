...
case "404"
if not convertBool(customer.bCustom404) then redirectToHP()
pageTitle=convertStr(customer.sCustom404Title)
pageTitle=replace(pageTitle,"[404FILENAME]",sanitize(request.querystring("404file")),1,-1,1)
selectedPage.sTitle=pageTitle
pageBody=convertStr(customer.sCustom404Body)
pageBody=replace(pageBody,"[404FILENAME]",sanitize(request.querystring("404file")),1,-1,1)
